...
object CommandManager : AbstractModule(), TabExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        root.invoke((command as PluginCommand).plugin as SpikotPlugin, CommandContext(sender, command, label, args.asList()), 0)
        return true
    }
...
class CommandHolder(private val commandHandler: KClass<out CommandHandler>) {
    ...
    internal fun invoke(plugin: SpikotPlugin, commandContext: CommandContext, depth: Int) {
        ...
        if (subCommand == null) {
            ...
            try {
                ...
                logger.catchAll("Cannot handle exception") {
    ...
    internal fun complete(plugin: SpikotPlugin, commandContext: CommandContext, depth: Int): List<String> {
        val subCommand = if (commandContext.args.size <= depth) {
            null
        } else {
